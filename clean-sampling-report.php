...
<head>
	...
	$page_title = 'Clean Sampling Report';
				...
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							...
							if(isset($_GET['query'])) {
								echo "<a href='clean-sampling-report.php' class='btn btn-link btn-sm mb-3 ml-2'><i class='fas fa-times mr-2'></i>Remove Filter</a>";
							}
					...
					<div class="collapse show" id="fsrd_report_container">
						<div class="card card-body">
							...
							if(isset($_POST['fsrd_report_form_btn'])) {
								...
								$stock_circle_id = stock_transaction_detail($stock_id, 'circle_id');
								...
								if( !empty($fscrd_report_id) ) {
									...
									if($report == 'rejected') {
										$fscrd_report_query = mysqli_query($conn, "INSERT INTO fscrd_report(user_id, circle_id, stock_id, report_type, time_created) VALUES('$user_id', '$stock_circle_id', '$stock_id', '3', '$time_created')");
									}
									...
									if($fscrd_report_query) {
										if($report_status == 1) {
											...
											$report_status_text = 'Rejected Clean Again';
										}
										echo "<div class='alert alert-success' id='fsrd_report_form_alert'>FSRD Successfully ".$report_status_text."</div>";
										echo "<script>history.pushState({}, '', 'clean-sampling-report.php'); setTimeout(function(){ $('#fsrd_report_container').removeClass('show'); $('#fsrd_comments').val(''); $('#fsrd_report_form_alert').remove(); }, 1000)</script>";
									} else {
										...
										<table class="table table-striped table-hover table-sm" id="table">
											...
											<tbody id="display_supply">
												...
												if(isset($_GET['query']) && $_GET['query'] == 'filter') {
													...
													$sql_query = "SELECT st.id AS stock_transaction_id, s.circle_id, s.stock_source, s.supplier_info, s.stock_qty, s.time_created, s.crop, s.variety, s.class, s.activity_season, f.id AS fscrd_report_id, f.report_comment, f.report_status FROM stocks AS s LEFT JOIN stock_transactions AS st ON s.id = st.stock_id LEFT JOIN (SELECT * FROM fscrd_report WHERE active_status = '1' AND delete_status = '0') AS f ON st.id = f.stock_id LEFT JOIN circles AS c ON s.circle_id = c.id WHERE f.report_type = '3' AND s.active_status = '1' AND f.active_status = '1' AND c.active_status = '1' AND s.delete_status = '0' AND f.delete_status = '0' AND c.delete_status = '0'";
													...
													// $sql_query = "SELECT st.id, s.circle_id, s.stock_source, s.supplier_info, s.stock_qty, s.time_created, s.crop, s.variety, s.class, s.activity_season FROM stocks AS s LEFT JOIN stock_transactions AS st LEFT JOIN fscrd_report AS f LEFT JOIN circles AS c ON s.id=st.stock_id && st.id=f.stock_id && s.circle_id=c.id WHERE (s.stock_source='from_farmer' || s.stock_source='others') && f.report_type='3' && s.active_status='1' && f.active_status='1' && c.active_status='1' && s.delete_status='0' && f.delete_status='0' && c.delete_status='0'";
													$sql_query = "SELECT st.id AS stock_transaction_id, s.circle_id, s.stock_source, s.supplier_info, s.stock_qty, s.time_created, s.crop, s.variety, s.class, s.activity_season, f.id AS fscrd_report_id, f.report_comment, f.report_status FROM stocks AS s LEFT JOIN stock_transactions AS st ON s.id = st.stock_id LEFT JOIN (SELECT * FROM fscrd_report WHERE active_status = '1' AND delete_status = '0') AS f ON st.id = f.stock_id LEFT JOIN circles AS c ON s.circle_id = c.id WHERE f.report_type = '3' AND s.active_status = '1' AND f.active_status = '1' AND c.active_status = '1' AND s.delete_status = '0' AND f.delete_status = '0' AND c.delete_status = '0'";

													if(is_admin()) {
														$sql_query .= " AND c.district='$user_district' ";
													}
													if(is_manager()) {
														$sql_query .= " AND s.circle_id='$circle_id' ";
													}
													$sql_query .= " ORDER BY s.time_created DESC ";
													// echo $sql_query;
													$query = mysqli_query($conn, $sql_query);
													...
													<tr>
														...
														<td>
															...
															if($result['report_status'] == 0) {
																if(is_admin()) {
																	echo "<div class='btn-group'><a href='?id=".$result['fscrd_report_id']."&stock_id=".$result['stock_transaction_id']."&action=fsrd_report&report=accepted' class='btn btn-success btn-sm clean_sample_accepted'>Accepted</a>";
																	echo "<a href='?id=".$result['fscrd_report_id']."&stock_id=".$result['stock_transaction_id']."&action=fsrd_report&report=rejected' class='btn btn-danger btn-sm clean_sample_rejected'>Clean Again</a></div>";
																}
																if(!is_admin()) {
																	echo "<span class='badge badge-secondary'>Pending</span>";
																}
															} else if($result['report_status'] == 1) {
																echo "<span class='badge badge-success'>Accepted</span>";
															} else if($result['report_status'] == 2) {
														...
														<td>
															...
															echo $result['report_comment'];
															if($result['report_status'] != 0) {
																if(is_admin()) {
																	echo "<span class='ml-2'><a href='?id=".$result['fscrd_report_id']."&action=fsrd_report'><i class='fas fa-edit'></i></a></span>";
																}
															}
														...
														/*$fscrd_report_query = mysqli_query($conn, "SELECT * FROM fscrd_report WHERE stock_id='$id' && report_type='3'");
														if(mysqli_num_rows($fscrd_report_query) > 0) {
															$fscrd_report_result = mysqli_fetch_assoc($fscrd_report_query);
															echo $fscrd_report_result['id'];
															echo "<td>";
															if($fscrd_report_result['report_status'] == 0) {
																if(is_admin()) {
																	echo "<div class='btn-group'><a href='?id=".$fscrd_report_result['id']."&stock_id=".$result['id']."&action=fsrd_report&report=accepted' class='btn btn-success btn-sm clean_sample_accepted'>Accepted</a>";
																	echo "<a href='?id=".$fscrd_report_result['id']."&stock_id=".$result['id']."&action=fsrd_report&report=rejected' class='btn btn-danger btn-sm clean_sample_rejected'>Clean Again</a></div>";
																}
																if(!is_admin()) {
																	echo "<span class='badge badge-secondary'>Pending</span>";
																}
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(document).on('click', '.fsrd_report_container_close', function(){
				history.pushState({}, '', 'clean-sampling-report.php');
			});
			...
			$(document).on('click', '.clean_sample_accepted', function(){
			...
			$(document).on('click', '.clean_sample_rejected', function(){
