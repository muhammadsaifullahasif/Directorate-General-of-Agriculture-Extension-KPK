				...
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							...
							if(isset($_GET['query'])) {
								echo "<a href='stock.php' class='btn btn-link btn-sm mb-3 ml-2'><i class='fas fa-times mr-2'></i>Remove Filter</a>";
							}
				...
				<div class="container-fluid">
					...
					if(isset($_GET['query'])) {
						...
						if(isset($_GET['filter_from_date']) && !empty( $_GET['filter_from_date'] ) && isset($_GET['filter_to_date']) && !empty( $_GET['filter_to_date'] ) ) {
							$search_result_msg .= ' from <b>'.date('d-M-Y', strtotime(validate($_GET['filter_from_date']))).'</b> to <b>'.date('d-M-Y', strtotime(validate($_GET['filter_to_date']))).date(' H:i:s').'</b>';
						...
						if(isset($_GET['filter_status']) && !empty( $_GET['filter_status'] ) ) {
							...
							} else if($_GET['filter_status'] == 'completed') {
								$search_result_msg .= ' <b>Completed</b>';
							}
						}
					...
					if(isset($_GET['action']) && $_GET['action'] == 'delete') {
						...
						if(isset($_GET['id']) && $_GET['id'] != 0 && !empty( $_GET['id'] ) ) {
							$id = validate($_GET['id']);
							// $finance_id = finance_id($circle_id);
							...
							if(mysqli_num_rows($check_query) > 0) {
								...
								$delete_stock_query = mysqli_query($conn, "DELETE FROM stock_transactions WHERE stock_id='$id' && circle_id='$stock_circle_id'");
								if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM stock_transactions WHERE stock_id='$id'")) == 0) {
									$delete_stock = mysqli_query($conn, "DELETE FROM stocks WHERE id='$id'");
									$delete_stock_meta = mysqli_query($conn, "DELETE FROM stock_meta WHERE stock_id='$id'");
								}
									...
									<form class="form" method="get">
										...
										<div class="row">
											...
											<div class="col-md-3 mb-3">
												...
												<select class="form-control filter_crop" id="filter_crop" name="filter_crop">
											...
											<div class="col-md-3 mb-3">
												...
												<select class="form-control" id="filter_status" name="filter_status">
													...
													<option <?php if(isset($_GET['filter_status']) && $_GET['filter_status'] == 'completed') echo 'selected'; ?> value="completed">Completed</option>
												</select>
											</div>
													...
													if(mysqli_num_rows($stock_crop_query) > 0) {
														while($stock_crop_result = mysqli_fetch_assoc($stock_crop_query)) {
															if(isset($_GET['filter_crop']) && $_GET['filter_crop'] != '') {
																if($_GET['filter_crop'] == $stock_crop_result['id']) {
																	$filter_crop_selected = 'selected';
																} else {
																	$filter_crop_selected = '';
																}
											...
											<tbody id="display_stocks">
												...
												if(isset($_GET['query'])) {
													...
													if($_GET['query'] == 'filter') {
														...
														if(isset($_GET['filter_activity_season']) && !empty( $_GET['filter_activity_season'] ) ) {
															...
															$sql_query .= " && s.activity_season='$filter_activity_season' ";
												...
												if(mysqli_num_rows($query) > 0) {
													...
													while($result = mysqli_fetch_assoc($query)) {
														$stock_id = $result['id'];
														$stock_transaction_sql = "SELECT SUM(stock_qty) AS stock_qty FROM stock_transactions WHERE stock_id='$stock_id' && delete_status='0'";
														...
														?>
														<tr>
															<td><a href="stock.php?query=filter&filter_crop=<?= $result['crop']; ?>"><?= stock_crop($result['crop']); ?></a></td>
															...
															<td>
																<div class="btn-group">
																	...
																	if(is_manager()) {
																	?>
																	<!-- <a href="stock-edit.php?id=<?= $result['id']; ?>" class="btn btn-primary btn-sm"><i class="fas fa-edit"></i></a> -->
																	<a href="stock.php?id=<?= $result['id']; ?>&action=delete" class="btn btn-danger btn-sm delete_stock_btn"><i class="fas fa-trash"></i></a>
																	<?php
																	}
																	?>
																</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(document).on('click', '.delete_stock_btn', function(){
				...
				if(confirm('Are you sure to delete stock, this process will never reverse, it will delete all the data associated with stock?')) {
					return true;
				} else {
					return false;
				}
