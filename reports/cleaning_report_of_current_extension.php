<table class="table table-bordered table-sm">
	<thead>
		<tr>
			<td colspan="13" class="text-center">
				<h3><b>VARIETY WISE SEED CLEANING REPORT <?php echo extension_name($report_extension).' '.activity_session_title($activity_session); ?></b></h3>
	...
	<tbody>
		...
		$report_variety_query = mysqli_query($conn, "SELECT sv.id, sv.variety FROM stock_variety AS sv INNER JOIN stocks AS s INNER JOIN stock_cleaning AS sc ON sv.id=s.variety && s.id=sc.parent_id WHERE s.extension_id='$report_extension' && s.type='$stock_type' && s.activity_session='$activity_session' && sv.stock_type_id='$stock_type' && sv.active_status='1' && s.active_status='1' && sc.active_status='1' && sv.delete_status='0' && s.delete_status='0' && sc.delete_status='0' GROUP BY s.variety");
		if(mysqli_num_rows($report_variety_query) > 0) {
			...
			while($report_variety_result = mysqli_fetch_assoc($report_variety_query)) {
				...
				$report_stock_query = mysqli_query($conn, "SELECT s.class, sc.id, SUM(sc.processing_qty) AS processing_qty FROM stocks AS s INNER JOIN stock_cleaning AS sc ON s.id=sc.parent_id WHERE s.extension_id='$report_extension' && s.type='$stock_type' && s.activity_session='$activity_session' && s.variety='$report_variety_id' && s.active_status='1' && sc.active_status='1' && s.delete_status='0' && sc.delete_status='0'");
				...
				if(mysqli_num_rows($report_class_query) > 0) {
					while($report_class_result = mysqli_fetch_assoc($report_class_query)) {
						...
						$report_class_stock_query = mysqli_query($conn, "SELECT sc.id FROM stocks AS s INNER JOIN stock_cleaning AS sc ON s.id=sc.parent_id WHERE s.extension_id='$report_extension' && s.type='$stock_type' && s.activity_session='$activity_session' && s.variety='$report_variety_id' && s.class='$report_class_id' && s.active_status='1' && sc.active_status='1' && s.delete_status='0' && sc.delete_status='0'");
				...
				while($total_variety_result = mysqli_fetch_assoc($total_variety_query)) {
					...
					if(mysqli_num_rows($total_class_query) > 0) {
						while($total_class_result = mysqli_fetch_assoc($total_class_query)) {
							...
							$total_class_stock_query = mysqli_query($conn, "SELECT sc.id FROM stocks AS s INNER JOIN stock_cleaning AS sc ON s.id=sc.parent_id WHERE s.extension_id='$report_extension' && s.type='$stock_type' && s.activity_session='$activity_session' && s.variety='$total_variety_id' && s.class='$total_class_id' && s.active_status='1' && sc.active_status='1' && s.delete_status='0' && sc.delete_status='0'");
