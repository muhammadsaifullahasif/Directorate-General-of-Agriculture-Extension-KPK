<div class="table-responsive">
	<table class="table table-bordered table-sm">
		<thead>
			<tr>
				<td colspan="100" class="align-middle text-center">
					<h4><b><?= stock_type($stock_type); ?> WITH IN DISTRICT ISSUE FROM <?= extension_name($extension_id); ?></b></h4>
					<span>Weight in <?= $stock_weight; ?></span>
				</td>
			</tr>
			<tr>
				<td rowspan="2" class="align-middle text-center"><b>S.No</b></td>
				<td rowspan="2" class="align-middle text-center"><b>Name of Extension</b></td>
				<?php
				...
				if(mysqli_num_rows($variety_query_1) > 0) {
					while($head_variety_result_1 = mysqli_fetch_assoc($variety_query_1)) {
						...
						$head_variety_class_query_1 = mysqli_query($conn, "SELECT sc.id FROM stock_class AS sc LEFT JOIN stocks AS s ON sc.id=s.class WHERE s.extension_id='$report_extension' && s.type='$stock_type' && s.activity_session='$activity_session' && s.variety='$head_variety_id_1' && sc.active_status='1' && s.active_status='1' && sc.delete_status='0' && s.delete_status='0' GROUP BY s.class");
		...
		<tbody>
			...
			$district_query = mysqli_query($conn, "SELECT c.id, s.receiver_detail FROM configurations AS c INNER JOIN extensions AS e INNER JOIN supply AS s ON c.id=e.district && e.id=s.receiver_detail WHERE c.type='district' && e.district='$user_district' && c.active_status='1' && e.active_status='1' && s.active_status='1' && c.delete_status='0' && e.delete_status='0' && s.delete_status='0' GROUP BY c.id");
			...
			if(mysqli_num_rows($district_query) > 0) {
				...
				while($district_result = mysqli_fetch_assoc($district_query)) {
					...
					echo "<td class='align-middle text-center'>".extension_name($district_result['receiver_detail'])."</td>";
