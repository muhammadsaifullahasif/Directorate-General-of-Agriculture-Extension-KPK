...
<head>
	...
	$page_title = 'Standing Crop Report';
							...
							if(isset($_POST['fsrd_report_form_btn'])) {
								...
								if( !empty($fscrd_report_id) ) {
									...
									if($fscrd_report_query) {
										...
										echo "<script>history.pushState({}, '', 'standing-crop-report.php'); setTimeout(function(){ $('#fsrd_report_container').removeClass('show'); $('#fsrd_comments').val(''); $('#fsrd_report_form_alert').remove(); }, 1000)</script>";
									} else {
										...
										<table class="table table-striped table-hover table-sm" id="table">
											...
											<tbody id="display_supply">
												...
												if(isset($_GET['query']) && $_GET['query'] == 'filter') {
													...
													if(isset($_GET['filter_status']) && !empty( $_GET['filter_status'] ) ) {
														$supply_sql .= " && f.report_status='".validate($_GET['filter_status'])."' ";
													}
													...
													if(isset($_GET['filter_farmer']) && !empty( $_GET['filter_farmer'] ) ) {
														$supply_sql .= " && ( (sp.receive_source='to_farmer' || sp.receive_source='others') && sp.receiver_detail='".validate($_GET['filter_farmer'])."' ) ";
													}
													...
													$supply_sql .= " ORDER BY sp.time_created DESC ";
												...
												if(mysqli_num_rows($query) > 0) {
													...
													<tr>
														...
														$fscrd_report_query = mysqli_query($conn, "SELECT * FROM fscrd_report WHERE stock_id='$id' && report_type='1'");
														if(mysqli_num_rows($fscrd_report_query) > 0) {
															...
															if($fscrd_report_result['report_status'] == 0) {
																if(is_admin()) {
																	echo "<div class='btn-group'><a href='?id=".$fscrd_report_result['id']."&action=fsrd_report&report=accepted' class='btn btn-success btn-sm smp_accepted'>Accepted</a>";
																	echo "<a href='?id=".$fscrd_report_result['id']."&action=fsrd_report&report=rejected' class='btn btn-danger btn-sm smp_rejected'>Rejected</a></div>";
																}
																if(!is_admin()) {
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(document).on('click', '.fsrd_report_container_close', function(){
				history.pushState({}, '', 'standing-crop-report.php');
			});
