...
<body class="hold-transition sidebar-mini layout-fixed layout-footer-fixed text-sm sidebar-collapse">
				...
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							...
							<a href="stock-fumigation-new.php" class="btn btn-outline-dark btn-sm mb-3 ml-2">Add New</a>
							<button class="btn btn-outline-dark btn-sm mb-3 ml-2" data-toggle="modal" data-target="#filter_modal">Advance Filter</button>
							<?php

							if(isset($_GET['query'])) {
								echo "<a href='stock-fumigation.php' class='btn btn-link btn-sm mb-3 ml-2'><i class='fas fa-times mr-2'></i>Remove Filter</a>";
							}
				...
				<div class="container-fluid">
					...
					if(isset($_GET['action']) && $_GET['action'] == 'delete') {
						...
						if(isset($_GET['id']) && $_GET['id'] != 0 && !empty( $_GET['id'] ) && isset($_GET['stock_id']) && $_GET['stock_id'] != 0 && !empty( $_GET['stock_id'] ) && isset($_GET['circle_id']) && $_GET['circle_id'] != 0 && !empty( $_GET['circle_id'] ) ) {
							...
							$check_stock_query = mysqli_query($conn, "SELECT * FROM stock_fumigation WHERE circle_id='$stock_circle_id' && stock_id='$stock_id' && active_status='1' && delete_status='0'");
							if(mysqli_num_rows($check_stock_query) == 1) {
								$check_stock_result = mysqli_fetch_assoc($check_stock_query);
								$processing_qty = $check_stock_result['processing_qty'];
								$stock_parent_id = $check_stock_result['parent_id'];
								$check_exist_stock_query = mysqli_query($conn, "SELECT * FROM stock_transactions WHERE circle_id='$stock_circle_id' && stock_id='$stock_parent_id' && active_status='1' && delete_status='0'");
								if(mysqli_num_rows($check_exist_stock_query) > 0) {
									$check_exist_stock_result = mysqli_fetch_assoc($check_exist_stock_query);
									$available_stock_id = $check_exist_stock_result['id'];
									$new_stock_qty = $check_exist_stock_result['stock_qty'] + $processing_qty;
									$stock_query = mysqli_query($conn, "UPDATE stock_transactions SET stock_qty='$new_stock_qty' WHERE id='$available_stock_id'");

									$del_stock_query = mysqli_query($conn, "DELETE FROM stock_transactions WHERE id='$stock_id'");

									$query = mysqli_query($conn, "DELETE FROM stock_fumigation WHERE id='$id' && stock_id='$stock_id'");
									$meta_query = mysqli_query($conn, "DELETE FROM stock_fumigation_meta WHERE stock_fumigation_id='$id'");
									$check_transaction_query = mysqli_query($conn, "SELECT * FROM transactions WHERE ref_id='$id' && type='1' && trans_flow='0'");
									if(mysqli_num_rows($check_transaction_query) > 0) {
										...
										$delete_transaction_query = mysqli_query($conn, "DELETE FROM transactions WHERE ref_id='$id' && type='1' && trans_flow='0'");
									}
								} else {
									
									$stock_query = mysqli_query($conn, "UPDATE stock_transactions SET active_status='1' WHERE id='$stock_id'");
										...
										<table class="table table-striped table-hover table-sm" id="table">
											...
											<tbody id="display_stocks">
												...
												if(isset($_GET['query'])) {
													...
													$sql_query = "SELECT st.id, st.circle_id, st.stock_id, s.crop, s.lot_number, s.variety, s.class, st.stock_qty, st.stock_status, st.active_status, sf.processing_qty FROM stocks AS s INNER JOIN stock_transactions AS st INNER JOIN stock_fumigation AS sf ON s.id=st.stock_id && s.id=sf.parent_id && st.id=sf.stock_id WHERE st.active_status='3' && (s.delete_status='0' && st.delete_status='0')";
													...
													$query = mysqli_query($conn, "SELECT sf.id, sf.stock_id, st.circle_id, s.crop, s.lot_number, s.variety, s.class, st.stock_qty, st.stock_status, st.active_status, sf.processing_qty FROM stocks AS s INNER JOIN stock_transactions AS st INNER JOIN stock_fumigation AS sf ON s.id=st.stock_id && st.id=sf.stock_id WHERE st.active_status='3' && (s.delete_status='0' && st.delete_status='0')");
														...
														<tr>
															...
															<td>
																<div class="btn-group">
																	...
																	?>
																	<a href="stock-fumigation.php?id=<?= $result['id']; ?>&stock_id=<?= $result['stock_id']; ?>&circle_id=<?= $result['circle_id']; ?>&action=delete" class="btn btn-danger btn-sm stock_fumigation_delete_btn"><i class="fas fa-trash"></i></a>
																	<?php
																		}
																	}
																	?>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(document).on('click', '.stock_fumigation_delete_btn', function(){

				if(confirm('Are you sure to delete fumigation?')) {
					return true;
				} else {
					return false;
				}
