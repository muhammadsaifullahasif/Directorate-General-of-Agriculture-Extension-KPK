		...
		<div class="content-wrapper">
			...
			<div class="content-header">
				<div class="container-fluid">
					<div class="row mb-2">
						<div class="col-sm-6">
							<h1 class="m-0 d-inline">SMP <?= $page_title; ?></h1>
							<?php
							...
							if(isset($_POST['fsrd_report_form_btn'])) {
								...
								if( !empty($supply_id) ) {
									...
									if($query && $meta_query) {
										...
										echo "<script>history.pushState({}, '', 'smp-list.php'); setTimeout(function(){ $('#fsrd_report_container').removeClass('show'); $('#fsrd_comments').val(''); $('#fsrd_report_form_alert').remove(); }, 1000)</script>";
									} else {
										...
										<table class="table table-striped table-hover table-sm" id="table">
											...
											<tbody id="display_supply">
												...
												if(isset($_GET['query']) && $_GET['query'] == 'filter') {
													...
													$supply_sql = "SELECT sp.id, sp.circle_id, sp.receive_source, sp.receiver_detail, sp.stock_qty, sp.activity_season, sp.receiver_info, sp.receiver_time_created, sp.receive_status, sp.time_created, s.crop, s.variety, s.class, sp.activity_season FROM supply AS sp INNER JOIN stocks AS s INNER JOIN circles AS e ON sp.parent_id=s.id && s.circle_id=e.id WHERE sp.receive_source='to_farmer' && sp.active_status='1' && s.active_status='1' && e.active_status='1' && sp.delete_status='0' && s.delete_status='0' && e.delete_status='0'";
													...
													if(isset($_GET['filter_status']) && !empty( $_GET['filter_status'] ) ) {
														$supply_sql .= " && sp.smp_status='".validate($_GET['filter_status'])."' ";
													}
												...
												if(mysqli_num_rows($query) > 0) {
													...
													<tr>
														...
														<td><a href="?query=filter&filter_type=<?= $result['crop']; ?>"><?= stock_crop($result['crop']); ?></a></td>
														...
														<!-- <td>
															...
															if($result['smp_status'] == 0) {
																if(is_admin()) {
																	echo "<a href='smp-list.php?id=".$result['id']."&action=fsrd_report&report=accepted' class='btn btn-success btn-sm smp_accepted'>Accepted</a>";
																	echo "<a href='smp-list.php?id=".$result['id']."&action=fsrd_report&report=rejected' class='btn btn-danger btn-sm smp_rejected'>Rejected</a>";
																}
															} else if($result['smp_status'] == 1) {
																echo "<span class='badge badge-success'>Accepted</span>";
															} else if($result['smp_status'] == 2) {
														...
														<td><?= supply_meta($result['id'], 'fsrd_comments'); if($result['smp_status'] != 0) { if(is_admin()) { echo "<span class='ml-2'><a href='smp-list.php?id=".$result['id']."&action=fsrd_report'><i class='fas fa-edit'></i></a></span>"; } } ?></td> -->
														<td><a href="form-1.php?id=<?= $result['id']; ?>&receiver_detail=<?= $result['receiver_detail']; ?>&activity_season=<?= $result['activity_season']; ?>" target="_blank" class="btn btn-primary btn-sm"><i class="fas fa-print mr-2"></i>Form 1</a></td>
													</tr>
													<?php
														$i++;
													}
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(document).on('click', '.fsrd_report_container_close', function(){
				history.pushState({}, '', 'smp-list.php');
			});
			...
			$(document).on('click', '.smp_accepted', function(){
			...
			$(document).on('click', '.smp_rejected', function(){
