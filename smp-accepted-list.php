							...
							if(isset($_POST['fsrd_report_form_btn'])) {
								...
								if( !empty($supply_id) ) {
									...
									if($query && $meta_query) {
										...
										echo "<script>history.pushState({}, '', 'smp-accepted-list.php'); setTimeout(function(){ $('#fsrd_report_container').removeClass('show'); $('#fsrd_comments').val(''); $('#fsrd_report_form_alert').remove(); }, 1000)</script>";
									} else {
												...
												if(mysqli_num_rows($query) > 0) {
													...
													<tr>
														...
														<td>
															...
															if($result['smp_status'] == 1) {
																echo "<span class='badge badge-success'>Accepted</span>";
															}

															?>
														</td>
														<td><?= supply_meta($result['id'], 'fsrd_comments'); if($result['smp_status'] != 0) { echo "<span class='ml-2'>"; if(is_admin()) { echo "<a href='smp-accepted-list.php?id=".$result['id']."&action=fsrd_report'><i class='fas fa-edit'></i></a></span>"; } } ?></td>
													</tr>
													<?php
														$i++;
													}
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			$(document).on('click', '.fsrd_report_container_close', function(){
				history.pushState({}, '', 'smp-accepted-list.php');
			});
